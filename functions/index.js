...
/** Shazam/Apple Music JWT (Callable getAppleMusicToken) — gleiche Secret-Namen wie bisher in runWith. */
...
/**
 * EmailJS Public-IDs — gleiche Werte wie Kontaktformular (PWA → validateRecaptchaAndSaveContact),
 * aus Firebase Runtime Config (emailjs.*). Als Fallback fest eingebaut, falls Env/Config fehlt.
 * Private Key bleibt nur in Secret [EMAILJS_PRIVATE_KEY].
 */
/** IONOS angebundener EmailJS-E-Mail-Dienst (Dashboard). */
const EMAILJS_FALLBACK_SERVICE_ID = 'service_dwdclwc';
const EMAILJS_FALLBACK_TEMPLATE_ID = 'template_upsc3oi';
const EMAILJS_FALLBACK_PUBLIC_KEY = '5ChQmGAWG7rTP-Q9K';

/** IONOS/SMTP: authentifizierte Absenderadresse (muss im Template als from_email genutzt werden). */
const EMAILJS_NOREPLY_FROM_EMAIL = 'noreply@vibesbox.app';

/**
 * Anzeigename Auth-Mails (from_name); physische Absenderadresse siehe [EMAILJS_NOREPLY_FROM_EMAIL].
 */
const EMAILJS_AUTH_MAIL_FROM_DISPLAY_NAME = 'VibesBox';

/**
 * EmailJS Service-/Template-ID und Public Key (kein Private Key).
 * Reihenfolge: `process.env.EMAILJS_*` (Console / .env) → Legacy `functions.config().emailjs` → Fallback-Konstanten.
 */
function getEmailJsPublicIds() {
  ...
  // IONOS: alte Service-ID aus functions.config ersetzen (sonst bliebe service_0e6txde aktiv).
  if (serviceId === 'service_0e6txde') {
    serviceId = EMAILJS_FALLBACK_SERVICE_ID;
  }
  if (!serviceId) {
    serviceId = EMAILJS_FALLBACK_SERVICE_ID;
  }
  if (!templateId) {
    templateId = EMAILJS_FALLBACK_TEMPLATE_ID;
  }
  if (!publicKey) {
    publicKey = EMAILJS_FALLBACK_PUBLIC_KEY;
  }
  return {
...
/** Web/PWA (muss zu lib/services/app_check_service.dart recaptchaWebSiteKey passen). */
const RECAPTCHA_ENTERPRISE_SITE_KEY =
  '6LdoeDcsAAAAAI0Rb90GjRovm2tW5qE4v9q5J-u5';
/** Android (muss zu app_check_service recaptchaAndroidSiteKey passen). */
const RECAPTCHA_ANDROID_SITE_KEY =
  '6Le9Wr4sAAAAAJwlB6cpaPZXHlam_QeRCvZkrrIW';
/** iOS / Legacy (optional). */
const RECAPTCHA_IOS_SITE_KEY =
  '6LdoeDcsAAAAAIORb90GjRovm2tW5qE4v9q5J-u5';

const ALLOWED_RECAPTCHA_SITE_KEYS = new Set([
  RECAPTCHA_ENTERPRISE_SITE_KEY,
  RECAPTCHA_ANDROID_SITE_KEY,
  RECAPTCHA_IOS_SITE_KEY,
  // Legacy: früher ein Key für alle Plattformen
  '6LdoeDcsAAAAAIORb90GjRovm2tW5qE4v9q5J-u5',
]);
/**
 ...
 * @param {string} [siteKeyOptional] — muss zu dem Key passen, mit dem das Token erzeugt wurde (Allowlist).
 */
async function verifyRecaptchaEnterpriseAssessment(
  ...
  const sk =
    siteKeyOptional &&
    typeof siteKeyOptional === 'string' &&
    ALLOWED_RECAPTCHA_SITE_KEYS.has(siteKeyOptional.trim())
      ? siteKeyOptional.trim()
      : RECAPTCHA_ENTERPRISE_SITE_KEY;
  const event = {
    token: token.trim(),
    siteKey: sk,
...
/**
 * MusicKit-/ShazamKit-Developer-Token (ES256), Standard 30 Min (1800 s).
 * Private Key nur in Secret Manager — nie in Firestore oder Client.
 */
...
function normalizeApplePrivateKey(rawValue) {
  // Unterstützt Secret-Formate mit escaped Newlines, CRLF und optionalen Wrapper-Quotes.
...
function validateApplePrivateKeyPem(pem) {
  return (
    typeof pem === 'string' &&
...
function buildAppleDeveloperToken(privateKey, keyId, teamId, expiresInSeconds = 1800) {
  const pem = typeof privateKey === 'string' ? privateKey.trim() : privateKey;
  ...
  const iat = payload && typeof payload.iat === 'number' ? payload.iat : Math.floor(Date.now() / 1000);
  const exp = payload && typeof payload.exp === 'number' ? payload.exp : iat + expiresInSeconds;
  return {
    token,
    issuedAt: iat,
    expiresAt: exp,
...
exports.validateRecaptchaAndSaveContact = onRequest(
  ...
  try {
    ...
    if (recipientEmail) {
      ...
      // DJ-/Kontakt: from_email = IONOS-noreply; from_name + reply_to = DJ (Anzeige & Antwort).
      const djEmailForIonos = (recipientEmail || '').trim() || 'info@vibesbox.app';
      ...
      const templateParams = {
        ...
        from_email: EMAILJS_NOREPLY_FROM_EMAIL,
        from_name: djEmailForIonos,
        reply_to: djEmailForIonos,
...
/**
 * Apple-Developer-Token nur für Admin oder DJ: users.admin === true oder roles/{role_id}.name ∈ {Admin, DJ}.
 * Keine hart codierten Rollen-IDs — gleiche Logik wie Client (role_id → roles.name).
 */
async function assertCallerIsAdminOrDj(request) {
  ...
  if (!roleId) {
    throw new HttpsError(
      'permission-denied',
      'Nur Admins und DJs dürfen die Musikerkennung mit Apple-Token nutzen.',
    );
  }
...
// ==========================================
// CALLABLE: Apple Music / Shazam — 30-Minuten-JWT aus Secrets (APPLE_PRIVATE_KEY / KEY_ID / TEAM_ID)
// ==========================================
// - Nur Admin/DJ (assertCallerIsAdminOrDj). Kein Firestore-Handshake; Party spielt keine Rolle.
// - enforceAppCheck: false — echte Geräte sonst oft mit App-Attestation blockiert.
exports.getAppleMusicToken = onCall(
  {
    region: 'us-central1',
    enforceAppCheck: false,
    secrets: [SECRET_APPLE_PRIVATE_KEY, SECRET_APPLE_KEY_ID, SECRET_APPLE_TEAM_ID],
  },
  async (request) => {
    await assertCallerIsAdminOrDj(request);

    const privateKeyRaw = SECRET_APPLE_PRIVATE_KEY.value() || '';
    const keyId = stripWrappingQuotes(
      SECRET_APPLE_KEY_ID.value() || '',
    );
    const teamId = stripWrappingQuotes(
      SECRET_APPLE_TEAM_ID.value() || '',
    );
    const privateKey = normalizeApplePrivateKey(privateKeyRaw);

    if (!privateKey) {
      throw new HttpsError('failed-precondition', 'APPLE_PRIVATE_KEY secret is empty or missing.');
    }
    if (!keyId || !teamId) {
      throw new HttpsError('failed-precondition', 'APPLE_KEY_ID or APPLE_TEAM_ID secret is empty or missing.');
    }
    if (!validateApplePrivateKeyPem(privateKey)) {
      throw new HttpsError(
        'failed-precondition',
        'APPLE_PRIVATE_KEY format invalid (expected PKCS8 PEM with BEGIN/END PRIVATE KEY).',
      );
    }

    try {
      const result = buildAppleDeveloperToken(privateKey, keyId, teamId, 1800);
      console.log('[getAppleMusicToken] ok (JWT issued, Admin/DJ — no token logged)');
      return {
        token: result.token,
...
/** Für EmailJS-Template (z. B. `{{oob_code}}`): oobCode aus Firebase-Auth-Link. */
function extractOobCodeFromAuthLink(link) {
...
async function sendAuthEmailViaEmailJS({
  ...
  const templateParams = {
    to_email: toEmail,
    from_email: EMAILJS_NOREPLY_FROM_EMAIL,
    // Registrierung/Passwort: fester Anzeigename (reply_to optional im Template)
    from_name: EMAILJS_AUTH_MAIL_FROM_DISPLAY_NAME,
    ...
    /** Einzelner Code für EmailJS (z. B. `{{oob_code}}`); bei Verifizierung/Reset gesetzt. */
    oob_code: oobCode && String(oobCode).trim() ? String(oobCode).trim() : '',
    role: roleLabel,
    party_info: '',
    created_at: dateFormatted,
    app_name: t.app_name,
...
exports.sendAuthEmail = onCall(
  ...
  async (request) => {
    ...
    const recaptchaSiteKeyClient =
      typeof data.recaptchaSiteKey === 'string' ? data.recaptchaSiteKey.trim() : '';
    const assessmentSiteKey =
      recaptchaSiteKeyClient && ALLOWED_RECAPTCHA_SITE_KEYS.has(recaptchaSiteKeyClient)
        ? recaptchaSiteKeyClient
        : null;
    const recaptchaTechnicalBypass = data.recaptchaTechnicalBypass === true;
    ...
    } else if (recaptchaTechnicalBypass) {
      console.warn(
        'sendAuthEmail: reCAPTCHA Enterprise technischer Bypass (kein Token; App Check weiterhin erzwungen)',
      );
    } else {
...
/**
 * Prüft nur, ob es für die deviceClientId eine aktive Party-/Gerätesperre gibt.
 * WICHTIG: Kein Schreiben auf users/{uid} (keine Account-Sperr-Fusion).
 */
...
exports.applyDeviceBlockFusion = onCall(
  ...
  async (request) => {
    ...
    return {
      fused: false,
      blocked: true,
      reason: 'device_block_active_party_only',
      party_id:
