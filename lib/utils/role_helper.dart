...
/// Alle Dokument-IDs in [roles], deren [name] exakt [roleName] ist.
/// Wichtig: In Firestore können mehrere Rollen-Docs dieselbe [name]-Bezeichnung haben;
/// [getRoleIdByName] liefert nur die erste — User-[role_id] kann auf eine andere zeigen.
Future<Set<String>> getAllRoleDocIdsByName(String roleName) async {
  try {
    final rolesSnapshot = await FirebaseFirestore.instance
        .collection('roles')
        .where('name', isEqualTo: roleName)
        .get();
    return rolesSnapshot.docs.map((d) => d.id).toSet();
  } catch (e) {
    debugLog('❌ Fehler getAllRoleDocIdsByName($roleName): $e');
    return {};
  }
}
