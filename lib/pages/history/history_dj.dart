...
class _HistoryDjPageState extends State<HistoryDjPage> {
  ...
  void _initializeTracksStream(String partyId, String? sessionId) {
    ...
    _tracksSubscription = FirebaseFirestore.instance
      ...
      debugLog('ANALYSE [History-Data]: ${snapshot.docs.length} Tracks, ${snapshot.docChanges.length} docChanges');
      // Inkrementell + Upsert pro trackId: kein clear() → weniger UI-Flackern; doppelte „added“
      // für dieselbe id (Cache/Server) erzeugen keine doppelten Zeilen.
      for (final change in snapshot.docChanges) {
        final doc = change.doc;
        final id = doc.id;
        if (change.type == DocumentChangeType.removed) {
          _accumulatedTracks.removeWhere((e) => e['trackId'] == id);
          continue;
        }
