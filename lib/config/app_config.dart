...
/// Abgeleitete Rolle **ausschließlich** durch Vergleich der User-[role_id]
/// mit den beim App-Start geladenen Rollen-Dokument-IDs (kein Filtern nach Namen).
enum AppRoleKind { admin, dj, guest, location, unknown }
...
class AppConfig {
  ...
  */
  ...
  /// Firestore-Rollen-ID für "Location" (wird in main() via getRoleIdByName('Location') gesetzt).
  static String? locationRoleId;

  /// Alle [roles]-Dokument-IDs mit passendem Namen (Duplikate/Alt-Docs). Ergänzt die Einzel-ID aus [getRoleIdByName].
  static Set<String> allAdminRoleDocIds = {};
  static Set<String> allDjRoleDocIds = {};
  static Set<String> allGuestRoleDocIds = {};
  static Set<String> allLocationRoleDocIds = {};

  /// True, sobald [main] alle Rollen-Sets aus Firestore gemerged hat — vorher kein strikter Rollen-Logout.
  static bool roleDocIdSetsHydrated = false;

  /// Signal für UI/Tests: parallel zu [roleDocIdSetsHydrated].
  static final ValueNotifier<bool> roleDocIdSetsHydrationReadyNotifier =
      ValueNotifier<bool>(false);

  /// Nach vollständigem Laden der Rollen-IDs in [main] aufrufen.
  static void markRoleDocIdSetsHydrated() {
    roleDocIdSetsHydrated = true;
    roleDocIdSetsHydrationReadyNotifier.value = true;
    debugLog(
      '👤 Rollen-Check: Sets geladen (Admin-IDs: ${allAdminRoleDocIds.length}), starte Validierung.',
    );
  }
  ...
  /// Setzt die Location-Rollen-ID (wird beim App-Start aus Firestore geladen).
  static void setLocationRoleId(String? id) {
    locationRoleId = id;
    if (id != null) {
      debugLog('✅ AppConfig: Location-Rollen-ID gesetzt: $id');
    }
  }

  /// Admin-, DJ- und Gast-ID sind geladen (Pflicht für gültige Session-Auswertung).
  static bool get coreRoleIdsConfigured =>
      (adminRoleId != null || allAdminRoleDocIds.isNotEmpty) &&
      (djRoleId != null || allDjRoleDocIds.isNotEmpty) &&
      (guestRoleId != null || allGuestRoleDocIds.isNotEmpty);

  static bool _idInSetOrSingleton(String? roleId, String? single, Set<String> all) {
    if (roleId == null || roleId.isEmpty) return false;
    if (single != null && roleId == single) return true;
    return all.contains(roleId);
  }

  /// Rolle aus [role_id] vs. allen bekannten Rollen-Dokument-IDs (inkl. Duplikate in [roles]).
  static AppRoleKind classifyRoleId(String? roleId) {
    if (roleId == null || roleId.isEmpty) return AppRoleKind.unknown;
    if (_idInSetOrSingleton(roleId, adminRoleId, allAdminRoleDocIds)) {
      return AppRoleKind.admin;
    }
    if (_idInSetOrSingleton(roleId, djRoleId, allDjRoleDocIds)) {
      return AppRoleKind.dj;
    }
    if (_idInSetOrSingleton(roleId, guestRoleId, allGuestRoleDocIds)) {
      return AppRoleKind.guest;
    }
    if (_idInSetOrSingleton(roleId, locationRoleId, allLocationRoleDocIds)) {
      return AppRoleKind.location;
    }
    return AppRoleKind.unknown;
  }

  /// Statische UI-Labels (nicht aus Firestore gelesen).
  static String? labelForRoleKind(AppRoleKind kind) {
    switch (kind) {
      case AppRoleKind.admin:
        return 'Admin';
      case AppRoleKind.dj:
        return 'DJ';
      case AppRoleKind.guest:
        return 'Gast';
      case AppRoleKind.location:
        return 'Location';
      case AppRoleKind.unknown:
        return null;
    }
  }
  ...
  static bool isAdminRole(UserModel? userModel) {
    ...
    final rid = userModel.roleId;
    return rid != null && _idInSetOrSingleton(rid, adminRoleId, allAdminRoleDocIds);
  }
  ...
  static bool isDjRole(UserModel? userModel) {
    if (userModel == null) return false;
    final rid = userModel.roleId;
    return rid != null && _idInSetOrSingleton(rid, djRoleId, allDjRoleDocIds);
  }

  /// Anzeige-Label aus [role_id] — nur [classifyRoleId], keine Namen aus Firestore.
  static String? roleDisplayNameFromRoleId(String? roleId) {
    return labelForRoleKind(classifyRoleId(roleId));
  }

  /// Gast-Konto — [role_id] in allen Gast-Rollen-Dokumenten.
  static bool isGuestRole(UserModel? userModel) {
    if (userModel == null) return false;
    final rid = userModel.roleId;
    return rid != null && _idInSetOrSingleton(rid, guestRoleId, allGuestRoleDocIds);
  }

  /// Location-Konto — [role_id] in allen Location-Rollen-Dokumenten.
  static bool isLocationRole(UserModel? userModel) {
    if (userModel == null) return false;
    final rid = userModel.roleId;
    return rid != null && _idInSetOrSingleton(rid, locationRoleId, allLocationRoleDocIds);
  }
