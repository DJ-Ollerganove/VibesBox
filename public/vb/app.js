    /** Party-Code aus ?code=, Pfad (/vb/p/…, /p/… wie Flutter-Deep-Link) oder Hash (#…?code=). */
    window.vbGetPartyCodeFromUrl = function vbGetPartyCodeFromUrl() {
      ...
      try {
        ...
        // Nach Root-Redirect (?code=): gleicher Tab, Code zusätzlich in session (Fallback wenn Query verloren geht)
        try {
    ...
    async function findPartyDocByJoinCode(partyCode) {
      ...
      await fetchDocs('party_code', partyCode);
      if (!isNaN(numericCode) && String(numericCode) === String(partyCode).trim()) await fetchDocs('party_code', numericCode);
      await fetchDocs('fixed_party_code', partyCode);
      if (!isNaN(numericCode) && String(numericCode) === String(partyCode).trim()) await fetchDocs('fixed_party_code', numericCode);
    ...
    async function getActivePartyInfo(partyCode) {
      try {
        ...
        if (partyCode && partyCode !== 'manual') {
          ...
          if (partyDoc) {
            ...
            if (endedByLifecycle || endedByTime) {
              if (window.IS_DEBUG) console.log('⚠️ Party mit Code "' + partyCode + '" ist beendet oder liegt in der Vergangenheit (Lifecycle/Zeitraum)');
            } else {
        ...
        // Zuerst prüfe, ob eine aktive Party läuft (inkl. lifecycle „upcoming“ — align mit Firestore partyGuestJoinCodeListAllowed)
        ...
        if (partyCode && partyCode !== 'manual') {
          ...
          try {
            ...
            if (partyDoc) {
              const data = partyDoc.data();
              var endedL2 = data.lifecycle_status === 'finished' || data.finished_at != null;
              var endedT2 = false;
              if (data.end_date && typeof data.end_date.toDate === 'function' && now > data.end_date.toDate()) endedT2 = true;
              if (endedL2 || endedT2) {
                if (window.IS_DEBUG) console.log('⚠️ Party mit Code "' + partyCode + '" ist beendet oder vorbei, wird nicht als aktiv zurückgegeben');
              } else {
                const partyName = data.party_name || null;
    ...
    async function checkWishboxStatus(options) {
      ...
      try {
        ...
        try {
          ...
          /** Gleicher Join-Code (z. B. Location): zuerst laufende Party, sonst nächste zukünftige (frühester Start). */
          let bestRunningSel = null;
          let bestFutureSel = null;
          let bestFutureStartMs = null;

          for (const partyDoc of partiesSnapshot.docs) {
            const data = partyDoc.data();
            const startTimestamp = data.start_date;
            const endTimestamp = data.end_date;
            const partyStatus = data.status || 'active';
            ...
            if (startTimestamp && endTimestamp) {
              const startDate = startTimestamp.toDate();
              const endDate = endTimestamp.toDate();
              if (!partyDataMatchesJoinCode(data, codeToCheck)) continue;

              const canonicalCode = data.party_code != null ? String(data.party_code) : codeToCheck;

              if (now >= startDate && now < endDate) {
                if (!bestRunningSel) bestRunningSel = { partyDoc, data, startDate, endDate, canonicalCode };
              } else if (now < startDate) {
                const sm = startDate.getTime();
                if (bestFutureStartMs == null || sm < bestFutureStartMs) {
                  bestFutureStartMs = sm;
                  bestFutureSel = { partyDoc, data, startDate, endDate, canonicalCode };
                }
              }
            }
          }
          ...
          if (bestRunningSel) {
            const partyDoc = bestRunningSel.partyDoc;
            const data = bestRunningSel.data;
            const canonicalCode = bestRunningSel.canonicalCode;
          ...
          if (bestFutureSel) {
            const partyDoc = bestFutureSel.partyDoc;
            const data = bestFutureSel.data;
            const startDate = bestFutureSel.startDate;
            const canonicalCode = bestFutureSel.canonicalCode;
            if (window.IS_DEBUG) console.log('⏰ Party startet in der Zukunft, zeige Pre-Party Wartemodus');
            currentPartyStartDate = startDate;
            ...
            localStorage.setItem('validatedPartyCode', canonicalCode);
            try { localStorage.removeItem('pending_party_code'); } catch (e) {}
            const partyName = data.name || 'Your Party';
            sessionStorage.setItem('validatedPartyName', partyName);
            localStorage.setItem('validatedPartyName', partyName);
            if (window.IS_DEBUG) console.log('💾 party_name gespeichert in sessionStorage:', partyName);
            if (window.IS_DEBUG) console.log('💾 party_name gespeichert in localStorage:', partyName);
    ...
    /** skipRedirect: nur Storage/UI, kein Redirect. endedNoticeForRoot: auf Root-PWA Hinweis „Party beendet“ anzeigen. */
            ...
            if (partyDoc.exists()) {
              ...
              if (startTimestamp && endTimestamp) {
                ...
                if (now < startDate) {
                  ...
                  updateWishboxUI();
                  await checkWishboxStatus();
                  var langCode = (typeof window.vbGetResolvedPwaLanguageCode === 'function') ? window.vbGetResolvedPwaLanguageCode() : '';
                  var tzId = partyData.timezone_id || 'UTC';
                  var startPosix = Math.floor(startDate.getTime() / 1000);
                  var clockRaw = (typeof window.formatPartyLocalTime === 'function') ? window.formatPartyLocalTime(startPosix, tzId, langCode) : '';
                  if (!clockRaw) {
                    try { clockRaw = startDate.toLocaleTimeString(typeof window.getPartyLocale === 'function' ? window.getPartyLocale(langCode) : undefined, { hour: '2-digit', minute: '2-digit' }); } catch (eClk) { clockRaw = ''; }
                  }
                  var tfTpl = (typeof getTranslation === 'function' ? getTranslation('time_format_clock') : null) || '{time}';
                  var clockDisplay = String(tfTpl).replace(/\{time\}/g, clockRaw);
                  var introFuture = (typeof getTranslation === 'function' ? getTranslation('main_party_not_started_intro') : null) || (typeof t === 'function' ? t('main_party_not_started_intro', 'This party has not started yet.') : 'This party has not started yet.');
                  var atTpl = (typeof getTranslation === 'function' ? getTranslation('main_party_starts_at') : null) || (typeof t === 'function' ? t('main_party_starts_at', 'Starts at {time} (local time)') : 'Starts at {time} (local time)');
                  var atLine = String(atTpl).replace(/\{time\}/g, clockDisplay);
                  var futureMsg = introFuture + '<br><br><strong>' + atLine + '</strong>';
                  var countdownStr = (typeof window.calculateTimeUntilParty === 'function' && typeof t === 'function') ? window.calculateTimeUntilParty(startPosix, t) : '';
                  ...
                  else window.history.replaceState({}, document.title, (typeof window.vbNormalizedWishboxPathname === 'function' ? window.vbNormalizedWishboxPathname() : window.location.pathname));
                  if (typeof window.vbApplyResolvedLanguageToUi === 'function') window.vbApplyResolvedLanguageToUi();
                  return true;
                }
              }
    ...
    setTimeout(async () => {
      // ✅ /vb/ nur mit Session-Erlaubnis (nach Root-Check) oder fortgesetzter Tab-Session — sonst Root (oder ?code= zur Prüfung)
      var vbBlockedByEntryGate = false;
      (function vbEnforceSessionEntryGate() {
        try {
          var p = (window.location.pathname || '').toLowerCase();
          if (p.indexOf('/vb') === -1) return;
          var gate = sessionStorage.getItem('vb_entry_granted') === '1';
          var sid = sessionStorage.getItem('validatedPartyId');
          var hasSessionParty = !!(sid && sid !== 'manual' && String(sid).trim() !== '');
          if (gate || hasSessionParty) return;
          var digits = '';
          try {
            if (typeof window.vbGetPartyCodeFromUrl === 'function') {
              var rc = window.vbGetPartyCodeFromUrl();
              if (rc != null && String(rc).trim() !== '') digits = String(rc).replace(/\D/g, '').substring(0, 8);
            }
          } catch (e1) {}
          if (digits.length < 8) {
            try {
              var sp = new URLSearchParams(window.location.search || '');
              var q = sp.get('code');
              if (q) digits = String(q).replace(/\D/g, '').substring(0, 8);
            } catch (e2) {}
          }
          vbBlockedByEntryGate = true;
          if (digits.length === 8) {
            window.location.replace('/?code=' + encodeURIComponent(digits));
            return;
          }
          window.location.replace('/');
        } catch (eGate) {}
      ...
      if (!qrLoginSuccess) {
        if (hadPartyCodeInUrl) {
          if (window.IS_DEBUG) console.log('DEBUG PWA: QR-/URL-Code vorhanden, Login fehlgeschlagen — bleibe in /vb/ (kein Redirect zu /).');
          if (typeof updateWishboxUI === 'function') updateWishboxUI();
          if (typeof checkWishboxStatus === 'function') checkWishboxStatus();
          return;
        }
        // Kein Code in der URL und kein erfolgreicher Auto-Login → zur Hauptseite (Marketing-PWA)
        if (window.IS_DEBUG) console.log('DEBUG PWA: processQRCodeLogin fehlgeschlagen ohne URL-Code, Redirect zu /');
        window.location.replace('/');
        return;
      }
    ...
    /** VibesBox-Standardkanäle nur außerhalb einer Party (Marketing), nicht für Gäste mit laufender Party. */
    ...
    async function renderSocialMediaLinks() {
      ...
      if (inParty && isFreeDj) {
        if (window.IS_DEBUG) console.log('⚠️ Free-DJ-Plan – keine Social-Links der DJ-Ebene, zeige Hinweis');
      ...
      if (!socialsData || !hasLinks) {
        if (window.IS_DEBUG) console.log(inParty ? '⚠️ Keine DJ-Social-Links – Hinweis für Party-Gast' : '⚠️ Keine Party – VibesBox-Fallback');
