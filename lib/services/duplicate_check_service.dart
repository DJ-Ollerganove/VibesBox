...
class DuplicateCheckService {
  ...
  /// Cache leeren (z. B. nach Admin-Änderung an `party_settings/current`), damit [ensurePartySettingsLoaded] neu lädt.
  static void invalidatePartySettingsCache() {
    _cachedThreshold = null;
    _cachedIgnoredKeywords = null;
  }

  /// Aktueller Duplikat-Schwellwert 0–1 (nach [ensurePartySettingsLoaded]); für History-Kurz-Dublette u. a.
  static double getCachedDuplicateThresholdOrFallback() {
    final t = _cachedThreshold ?? 0.85;
    return t.clamp(0.5, 0.98);
  }
  ...
  /// Bei vollständigem Titel+Künstler auf beiden Seiten: Minimum (strenger als Durchschnitt).
  static double _combinedSimilarityForDuplicate(
    double titleSimilarity,
    double artistSimilarity, {
    required bool hasTitlePair,
    required bool hasArtistPair,
  }) {
    if (hasTitlePair && hasArtistPair) {
      return min(titleSimilarity, artistSimilarity);
    }
    if (hasTitlePair) return titleSimilarity;
    if (hasArtistPair) return artistSimilarity;
    ...
    try {
      ...
      try {
        ...
        for (final sessionDoc in historySessions.docs) {
          ...
          for (final trackDoc in tracksSnapshot.docs) {
            ...
            final hasTP = normalizedTitle.isNotEmpty && trackTitle.isNotEmpty;
            final hasAP = normalizedArtist.isNotEmpty && trackArtist.isNotEmpty;
            final avgSimilarity = _combinedSimilarityForDuplicate(
              titleSimilarity,
              artistSimilarity,
              hasTitlePair: hasTP,
              hasArtistPair: hasAP,
      ...
      try {
        ...
        for (final wishDoc in playedWishesSnapshot.docs) {
          ...
          final hasTP = normalizedTitle.isNotEmpty && wishTitle.isNotEmpty;
          final hasAP = normalizedArtist.isNotEmpty && wishArtist.isNotEmpty;
          final avgSimilarity = _combinedSimilarityForDuplicate(
            titleSimilarity,
            artistSimilarity,
            hasTitlePair: hasTP,
