...
class WishCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final rawTitle = (data['title'] ?? data['song'] ?? '') as String;
    ...
    final titleVariants = List<String>.from(
      (data['title_variants'] as List?)?.whereType<String>() ?? [],
    ).map(unescapeHtml).toList();
  ...
  SongRequest _mapToRequest(Map<String, dynamic> data) {
    return SongRequest(
      id: '',
      title: unescapeHtml((data['title'] ?? data['song'] ?? '') as String),
      artist: unescapeHtml((data['artist'] ?? '') as String),
      name: unescapeHtml((data['name'] ?? '') as String),
      greeting: unescapeHtml((data['greeting'] ?? '') as String),
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(AppLocalizations.of(context)!.favorite_status_update_error),
