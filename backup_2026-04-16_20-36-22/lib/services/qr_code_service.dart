...
class QrCodeService {
  /// Formatiert DateTime für Bilder
  static String formatDateTimeForImage(DateTime date) {
  ...
  /// Formatiert nur Uhrzeit für Header (HH:mm Uhr)
  static String formatTimeForImage(DateTime date) {
    final hour = date.hour.toString().padLeft(2, '0');
    final minute = date.minute.toString().padLeft(2, '0');
  ...
  static Future<bool> saveQRCodeAsImage(
    ...
    final partyCodeLabel = translations['party_code_label'] ?? 'Party-Code:';
    final contactLabel = translations['contact_label'] ?? 'Kontakt';
    final startTimeLabel = translations['party_start_label'] ?? 'Start:';
    final scanLine1 = translations['scan_qr_code_line1'] ?? 'Scan den QR Code';
    final scanLine2 = translations['scan_qr_code_line2'] ?? 'für Deinen Musikwunsch';
    ...
    if (logoImage != null) {
      ...
      final djNameFallback = TextPainter(
        text: TextSpan(text: djName ?? 'DJ', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: UIConstants.colorWhite)),
        textDirection: ui.TextDirection.ltr,
        maxLines: 2,
      );
    ...
    if (hasContactBlock) {
      ...
      final leftNameP = TextPainter(text: TextSpan(text: djName ?? 'DJ', style: djNameStyle), textDirection: ui.TextDirection.ltr, maxLines: 2);
    ...
    if (result['isSuccess'] == true) {
      ...
      throw Exception('Fehler beim Speichern in der Galerie');
    }
  }
}
