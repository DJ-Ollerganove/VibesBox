    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${docIds.length} Wünsche wurden aktualisiert')),
        );
      }
    } catch (e) {
      if (context.mounted) {
  ...
  Future<void> _confirmDeleteGrouped(BuildContext context, List<String> docIds, String displayText) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Möchtest du alle ${docIds.length} Wünsche für "$displayText" wirklich endgültig löschen?'),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('${docIds.length} Wünsche wurden gelöscht')),
          );
        }
      } catch (e) {
        if (context.mounted) {
