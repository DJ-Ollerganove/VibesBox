...
/// Root-[NavigatorState]: hält den Stack stabil, wenn [MaterialApp] (z. B. Locale-Update) neu gebaut wird –
/// verhindert Neu-Mount von [MainPage] und „Silent Reset“.
...
Future<void> main() async {
  ...
  // RevenueCat (nur Mobile: Android/iOS)
  ...
  if (!kIsWeb) {
    try {
      ...
      // Keine harte Abhängigkeit: App darf weiter starten, auch wenn Billing/Store nicht verfügbar ist.
  ...
  // Local Notifications werden erst im DJ-Bereich initialisiert (main_main_page.dart, _onUserModelChanged)

  // Initialisiere ActivePartyService (Heartbeat, Auto-Resume)

  try {
    await ActivePartyService.initialize();

    debugLog('✅ ActivePartyService initialisiert');
  } catch (e) {
    debugLog('⚠️ Fehler beim Initialisieren des ActivePartyService: $e');
  }

  // Initialisiere HistoryProvider (Musikerkennung-Historie)
  ...
  try {
    ...
    debugLog('✅ HistoryProvider initialisiert');
  } catch (e) {
    debugLog('⚠️ Fehler beim Initialisieren des HistoryProvider: $e');
  }
  ...
  // EINMALIGES CLEANUP-SCRIPT: Bereinigt Altlasten in Firestore parties Collection
  // WICHTIG: Nach erfolgreichem Durchlauf wieder entfernen!
  try {
    // Führe Cleanup mit Verzögerung aus (blockiert App-Start nicht)
    Future.delayed(const Duration(seconds: 2), () async {
      await PartyCleanupService.runCleanupOnce();
    });
  } catch (e) {
    debugLog('⚠️ Fehler beim Starten des Party-Cleanup: $e');
  }
...
/// Initialisiert den Notification-Service (flutter_local_notifications).
/// Wird aus main_main_page.dart aufgerufen, sobald ein DJ/Admin/Location erkannt wird – nicht beim App-Start.
Future<void> initializeNotifications() async {
  ...
  // Auf false: Berechtigungen werden später manuell im DJ-Bereich angefragt (nicht beim App-Start)
  ...
  if (Platform.isAndroid) {
    ...
    if (androidPlugin != null) {
      ...
      try {
        ...
        debugLog('🔄 Alter Notification Channel gelöscht');
        ...
        debugLog(
          'ℹ️ Channel existiert noch nicht oder konnte nicht gelöscht werden: $e',
        );
      }
      ...
      debugLog('✅ Notification Channel mit benutzerdefiniertem Sound erstellt');

      debugLog('   Sound-Datei: notification.mp3');
    }
  }
}
