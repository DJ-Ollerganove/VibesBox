...
class HistoryProvider {
  ...
  void initialize() {
    ...
    if (user != null) {
      // Abonniere Partys des aktuellen DJs als Stream
    ...
    // Lausche auf Musikerkennungs-Status-Änderungen (für Session-Updates)
    _statusSubscription = _shazamService.statusStream.listen((_) {
      _checkAndUpdateSession();
    });
    
    // Lausche auf erkannte Songs - speichere nur wenn Session aktiv ist (Party-basiert)
    // WICHTIG: resultStream sendet IMMER Ergebnisse (auch ohne Party) für UI-Anzeige
    // Aber _shouldRecord() verhindert Speichern ohne aktive Session (Test-Modus)
    _shazamSubscription = _shazamService.resultStream.listen((result) {
      ...
      // Wenn _shouldRecord() false ist: Song wird nur im UI angezeigt (ShazamFooter),
      // aber NICHT in Firestore gespeichert (Test-Modus)
    });
    
    // Initiale Prüfung
    _checkAndUpdateSession();
    
    // ✅ EFFIZIENZ: Tracks-Stream wird NICHT automatisch gestartet
    // Wird erst gestartet, wenn startHistoryListening() explizit aufgerufen wird (z.B. beim Öffnen der History-Seite)
    // Sende leere Liste, damit StreamBuilder nicht endlos wartet
  ...
  bool _shouldRecord() {
    ...
    final shouldRecord = _isRecording && _currentSessionId != null && _currentPartyId != null && _currentPartyId != 'manual' && _currentPartyId!.isNotEmpty;
    
    if (!shouldRecord) {
      // Debug-Log bei Bedarf: Warum wird nicht gespeichert?
      if (_currentPartyId == null || _currentPartyId == 'manual') {
        debugLog('⚠️ HistoryProvider: Song wird NICHT gespeichert – keine aktive Party (party_id: $_currentPartyId)');
      } else if (!_isRecording || _currentSessionId == null) {
        debugLog('⚠️ HistoryProvider: Song wird NICHT gespeichert – Session nicht aktiv (recording: $_isRecording, sessionId: $_currentSessionId)');
      }
    }
  ...
  Future<void> _checkAndAddTrack(TrackEntry newTrack) async {
    ...
    try {
      ...
      // 1) Zuerst nur den jüngsten Track laden — Basis für shouldSkip / analyze
      ...
      if (lastTrack != null) {
        ...
        if (analysis.shouldSkip) {
          debugLog(
            'History: Dublette ignoriert (${(analysis.avgSimilarity * 100).toStringAsFixed(0)}% Match, '
  ...
  /// Es werden nur DocumentChangeType.added (und .removed für Konsistenz) verarbeitet.
      ...
      _tracksSubscription = _firestore
        ...
        for (final change in snapshot.docChanges) {
          if (change.type == DocumentChangeType.added) {
            ...
            if (data != null) {
              final track = TrackEntry.fromFirestore(data);
            ...
            _accumulatedTracksForStream.removeWhere((e) => e['trackId'] == change.doc.id);
          }
        }
