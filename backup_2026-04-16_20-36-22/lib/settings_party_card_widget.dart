...
class _SettingsPartyCardState extends State<SettingsPartyCard> {
        ...
        final cardContent = Column(
          ...
          children: [
                ...
                FutureBuilder<Map<String, String?>>(
                  future: _calculatePartyTimes(),
                  builder: (context, timesSnapshot) {
                    if (!timesSnapshot.hasData) {
                      // Fallback: Zeige normale Formatierung
                      final startTimeFormatted = _formatDateCompact(widget.startDate);
                      final endTimeFormatted = _formatDateCompact(widget.endDate);
                      return RichText(
                        text: TextSpan(
  ...
  Future<Map<String, String?>> _calculatePartyTimes() async {
    try {
      if (!_timezoneInitialized) {
      ...
      // Fallback-Werte
      String startPartyTime = _formatDateCompact(widget.startDate);
      String endPartyTime = _formatDateCompact(widget.endDate);
      ...
      if (widget.startTimePosix != null && 
        ...
        try {
          ...
          startPartyTime = _formatDateCompact(DateTime(
            startPartyTZ.year,
            startPartyTZ.month,
            startPartyTZ.day,
            startPartyTZ.hour,
          ...
          // Konvertiere TZDateTime zu DateTime für Formatierung
          endPartyTime = _formatDateCompact(DateTime(
            endPartyTZ.year,
            endPartyTZ.month,
            endPartyTZ.day,
            endPartyTZ.hour,
          ...
          if (startPartyOffset != startDeviceOffset) {
            startDeviceTime = DateFormat('HH:mm').format(startLocalDateTime);
          }
          
          if (endPartyOffset != endDeviceOffset) {
            endDeviceTime = DateFormat('HH:mm').format(endLocalDateTime);
          }
      ...
      // Fallback
      return {
        'startPartyTime': _formatDateCompact(widget.startDate),
        'startDeviceTime': null,
        'endPartyTime': _formatDateCompact(widget.endDate),
        'endDeviceTime': null,
      };
    }
  }
  ...
  /// Formatiert Datum im kompakten Format: DD.MM. HH:mm Uhr
  ...
  Future<void> _openGoogleMaps() async {
    try {
      ...
      if (lat != null && lng != null && lat != 0.0 && lng != 0.0) {
        ...
        if (!fallbackLaunched && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Konnte keine Karten-App öffnen'),
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Keine Standortdaten verfügbar'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Fehler beim Öffnen der Karten-App: $e'),
            backgroundColor: Colors.red,
