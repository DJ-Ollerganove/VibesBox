...
class _TotalStatisticsCardState extends State<TotalStatisticsCard> {
  ...
  @override
  void didUpdateWidget(covariant TotalStatisticsCard oldWidget) {
  ...
  /// Beendete Partys dieses DJs: jede Änderung triggert neuen Abruf (debounced).
  void _subscribeFinishedParties() {
    _finishedPartiesSub?.cancel();
    _finishedPartiesSub = FirebaseFirestore.instance
        .collection('parties')
  ...
  void _scheduleReloadAggregates() {
    ...
    _reloadDebounce = Timer(const Duration(milliseconds: 500), () {
      if (mounted) {
        unawaited(_reloadAggregates(showLoadingSpinner: false));
      }
    });
  }

  /// Lädt Summen aus beendeten Partys + Party-Anzahl; [showLoadingSpinner] nur beim ersten Mal.
  Future<void> _reloadAggregates({required bool showLoadingSpinner}) async {
    if (!mounted) return;
            ...
            Text.rich(
              TextSpan(
                children: [
                  ...
                  if (hasActiveParty)
                    TextSpan(
                      text: l.dj_home_total_balance_note_excluding_active,
                      style: const TextStyle(
                        fontSize: 8,
          ...
          child: Column(
              ...
              children: [
                  ...
                  Row(
                    ...
                    children: [
                      Text(
                        l.dj_home_stats_row_party_count,
                        style: const TextStyle(
                          fontSize: 12,
                          color: Colors.white,
                        ),
                  ...
                  if (stats.averageWaitTime != null) ...[
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Text(
                          '${stats.averageWaitTime!.toStringAsFixed(0)} ${l.interval_minutes_short}',
                          style: const TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.bold,
                            color: UIConstants.appOrange,
