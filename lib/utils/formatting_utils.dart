...
class FormattingUtils {
  ...
  static String formatStartTimeForExport(DateTime date, Locale locale) {
    ...
    if (use12Hour) {
      ...
      final amPm = date.hour < 12 ? LocaleHelper.tr(t, 'time_am') : LocaleHelper.tr(t, 'time_pm');
  ...
  static String formatDateTimeForLanguageExport(DateTime date, String languageCode) {
    ...
    final tag = Locale(code).toString();
    return '${intl.DateFormat.yMd(tag).format(date)} ${intl.DateFormat.jm(tag).format(date)}';
  }
  ...
  static String formatDateForLocale(DateTime date, BuildContext context) {
    ...
    return intl.DateFormat.yMd(locale.toString()).format(date);
  }
  ...
  static String formatCompactDateTimeWithoutContext(DateTime dateTime) {
    ...
    return '${intl.DateFormat.yMd(tag).format(dateTime)} ${intl.DateFormat.jm(tag).format(dateTime)}';
  }

  /// Nur Uhrzeit, Geräte-Locale (Fallback ohne [BuildContext]).
  static String formatShortTimeWithoutContext(DateTime dateTime) {
    final loc = WidgetsBinding.instance.platformDispatcher.locale;
    return intl.DateFormat.jm(loc.toString()).format(dateTime);
  }
  ...
  static String formatCompactPartyPeriod(
    ...
    final startStr =
        '${intl.DateFormat.yMd(tag).format(start)} ${intl.DateFormat.jm(tag).format(start)}';
    if (end == null) return startStr;
    final endStr =
        '${intl.DateFormat.yMd(tag).format(end)} ${intl.DateFormat.jm(tag).format(end)}';
