...
class DeviceBlockFusionService {
  ...
  /// Android [AndroidDeviceInfo.id] (Build/Device-Id aus Log) — Fusion für Tests überspringen,
  /// damit kein `gesperrt`-Status den Mail-/Callable-Flow blockiert.
  static const Set<String> _applyFusionBypassAndroidIds = {
    'BP2A.250605.031.A3',
  };
  ...
  static Future<void> applyIfNeeded(User user) async {
    try {
      if (!kIsWeb && Platform.isAndroid) {
        final a = await DeviceInfoPlugin().androidInfo;
        final rawId = a.id.trim();
        if (_applyFusionBypassAndroidIds.contains(rawId)) {
          debugLog(
            'DeviceBlockFusion: übersprungen (Bypass-Liste, Gerät $rawId).',
          );
          return;
        }
      }
    } catch (e, st) {
      debugLog('DeviceBlockFusion bypass check: $e\n$st');
    }
