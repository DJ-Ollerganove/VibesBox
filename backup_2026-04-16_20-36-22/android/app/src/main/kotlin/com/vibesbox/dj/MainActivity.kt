...
class MainActivity: FlutterFragmentActivity() {
        ...
        shazamChannel.setMethodCallHandler { call, result ->
            when (call.method) {
                ...
                "startScanning" -> {
                    ...
                    startForegroundService(showStatusNotification, notificationTitle, notificationListening)
                    result.success(true)
                }
                "stopScanning" -> {
                    stopForegroundService()
    ...
    private fun startShazamRecognition() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            try {
                ...
                // Debug: Zeige Token-Info
                if (token.isNotEmpty()) {
                    Log.d("Shazam", "Token verwendet: Länge=${token.length}, Start=${token.take(10)}...")
                }
                                    ...
                                    try {
                                        when (matchResult) {
                                            ...
                                            is MatchResult.Error -> {
                                                ...
                                                Log.e("Shazam", "Match-Fehler: $errorMessage")
    ...
    private fun startForegroundService(showStatusNotification: Boolean, notificationTitle: String?, notificationListening: String?) {
