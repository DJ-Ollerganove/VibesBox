...
/// `planType == pro`) Social-Links einmalig geladen und im Service gespeichert.
/// Keine weiteren Firestore-Abfragen auf den Gast-Seiten – alles kommt aus dem "Session-Koffer".
///
...
class PartySessionService {
  ...
  /// DJ hat einen zahlungspflichtigen o. ä. Plan (nicht free) — z. B. Logo, Kontakt.
  bool get isPro =>
      _djPlan != null && _djPlan!.toString().toLowerCase() != 'free';

  /// Streng für Gast-Social: nur `planType == pro` aus dem öffentlichen DJ-Profil.
  /// Keine DJ-Links bei free/trial — unabhängig von alten Firestore-Einträgen.
  ...
  Future<PartyCheckInFeedback?> _validateAndSaveFromPartyDoc(
    ...
    // Alles-drin: Social-Links nur bei planType pro (nicht trial/free)
    Map<String, String> socialLinks = {};
    List<String> linkOrder = [];
    final loadDjSocial =
        djPlan.toString().toLowerCase() == 'pro';
    if (loadDjSocial) {
  ...
  Future<bool> hydrateIfNeeded() async {
    ...
    try {
      ...
      if (_hasCompleteSession) {
        // Fehlende Social-Links nachladen (z.B. nach App-Kill), nur bei Pro-Plan
        if (isDjProPlanForSocialLinks &&
            _djId != null &&
