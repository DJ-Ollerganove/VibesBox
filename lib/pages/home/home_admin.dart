...
class _HomeAdminState extends State<HomeAdmin> {
  ...
  final TextEditingController _newLangNameController = TextEditingController();
  bool _savingNewLanguage = false;
  ...
  void _showLastAnnouncement(BuildContext context) {
    ...
    showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(
          subject.isEmpty
              ? AppLocalizations.of(context)!.admin_dialog_last_announcement_empty_title
              : subject,
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          backgroundColor: const Color(0xFF1F2937),
          title: Text(
            loc.admin_announcement_delete_title,
            style: const TextStyle(color: Colors.white),
          ),
          content: Text(
            loc.admin_announcement_delete_confirm_body,
            style: const TextStyle(color: Colors.white70),
          ),
          actions: [
            TextButton(
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    l.admin_last_announcement_manage_title,
                    maxLines: 1,
            ...
            if (_loadingLastAnnouncement)
              ...
              Column(
                ...
                children: [
                  Text(
                    _lastAnnouncementSubjectPreview(context),
                  ...
                  Wrap(
                    ...
                    children: [
                      TextButton.icon(
                        ...
                        label: Text(
                          l.admin_show_last_announcement_button,
                          style: const TextStyle(color: Colors.orange),
                        ),
                      ),
                      TextButton.icon(
  ...
  Future<void> _saveNewLanguageEntry() async {
    if (UserScope.userOf(context)?.admin != true) return;
    final l = AppLocalizations.of(context)!;
    final code = SecurityHelper.sanitize(
      _newLangCodeController.text,
      maxLength: 8,
    );
    final nameEn = SecurityHelper.sanitize(
      _newLangNameController.text,
      maxLength: 80,
    );
    ...
    setState(() => _savingNewLanguage = true);
    try {
      await AnnouncementLanguagesService.addLanguage(
        codeRaw: code,
        nameEnglish: nameEn,
      );
      if (!mounted) return;
      ...
      if (mounted) setState(() => _savingNewLanguage = false);
    }
  }

  Widget _buildAddLanguageCard(BuildContext context) {
    final l = AppLocalizations.of(context)!;
    return widget.cardBuilder(
      context,
      Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              l.admin_language_new_entry_title,
              style: Theme.of(context).textTheme.titleMedium?.copyWith(
                    color: Colors.white,
                    fontWeight: FontWeight.bold,
                  ),
            ...
            Text(
              'Nur englischer Name und Kürzel; js_path und dart_file werden automatisch wie bei den anderen Einträgen gesetzt.',
            ...
            const SizedBox(height: 12),
            TextField(
              controller: _newLangNameController,
              decoration: const InputDecoration(
                labelText: 'Name der Sprache (Englisch)',
                hintText: 'e.g. Polish',
            ...
            const SizedBox(height: 8),
            TextField(
              controller: _newLangCodeController,
              decoration: const InputDecoration(
                labelText: 'Kürzel (Kleinbuchstaben)',
                hintText: 'pl',
                border: OutlineInputBorder(),
                focusedBorder: OutlineInputBorder(
                  borderSide: BorderSide(color: Colors.orange, width: 1.5),
            ...
            const SizedBox(height: 12),
            FilledButton.icon(
              onPressed: _savingNewLanguage ? null : () => unawaited(_saveNewLanguageEntry()),
              icon: _savingNewLanguage
                  ? const SizedBox(
                      width: 18,
                      height: 18,
              ...
              label: Text(
                _savingNewLanguage ? l.admin_language_saving : l.admin_language_save,
              ),
              style: FilledButton.styleFrom(
                backgroundColor: Colors.orange,
    ...
    return widget.cardBuilder(
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            Text(
              l.admin_set_default_start_view_title,
              style: Theme.of(context).textTheme.titleMedium?.copyWith(
                color: Colors.white,
                fontWeight: FontWeight.bold,
              ),
            ...
            DropdownButtonFormField<String>(
              ...
              decoration: InputDecoration(
                labelText: l.admin_start_view_label,
              ...
              items: [
                DropdownMenuItem(
                  ...
                  child: Text(l.admin_start_view_option_admin_dashboard),
                ),
                DropdownMenuItem(
                  value: 'dj_area',
                  child: Text(l.admin_start_view_option_dj_area),
                ),
                DropdownMenuItem(
                  value: 'guest_area',
                  child: Text(l.admin_start_view_option_guest_area),
                ),
              ],
              onChanged: (value) {
  ...
  String _lastAnnouncementSubjectPreview(BuildContext context) {
    final doc = _lastAnnouncementDoc;
    if (doc == null) return '';
    final data = doc.data() as Map<String, dynamic>?;
    final translations = _safeTranslationsMap(data);
    ...
    final subject = (de['subject'] as String?)?.trim() ?? '';
    if (subject.isEmpty) {
      return AppLocalizations.of(context)!.admin_announcement_no_subject_preview;
    }
    return subject;
  }
  ...
  Future<void> _saveDuplicateThreshold() async {
    ...
    try {
      ...
      DuplicateCheckService.invalidatePartySettingsCache();
    } catch (_) {}
  }
