...
class RemoteConfigService {
  ...
  static Future<void> initialize() async {
    ...
    try {
      ...
      _initialized = true;
      debugLog('✅ Remote Config Defaults gesetzt (Server-Fetch asynchron)');
      debugLog('   - enable_app_check: ${getBool('enable_app_check')}');
      debugLog('   - cooldown_seconds: ${getInt('cooldown_seconds')}');

      // fetchAndActivate blockiert den Start spürbar — im Hintergrund nachziehen
      unawaited(
        _remoteConfig!.fetchAndActivate().then((_) {
          debugLog('✅ Remote Config fetchAndActivate abgeschlossen');
        }).catchError((Object e) {
          debugLog('⚠️ Remote Config fetchAndActivate: $e');
        }),
      );
    } catch (e) {
