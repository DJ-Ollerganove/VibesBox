...
/// E-Mail/Passwort-Konto ohne bestätigte Adresse → Verifizierungs-Schranke (nicht Profil-Lade-/Logout-Pfad).
/// Basiert auf Provider `password`, nicht auf `email != null` (sonst fällt die UI fälschlich durch).
bool _userNeedsEmailVerificationBarrier(User? u) {
...
class _MainPageState extends State<MainPage>
  ...
  String? _sessionLockedRoleId;
  String?
  _currentRoleName; // Nur aus AppConfig.classifyRoleId → Label (kein roles.name)
  ...
  /// Nach Login/Kaltstart: erst [User.reload], dann Entscheidung Verify-Gate (verhindert UI-Flackern).
  String? _emailVerificationCheckedUid;
  bool _emailVerificationCheckScheduled = false;
  bool _emailVerificationCheckInProgress = false;

  /// [ActivePartyService] / [HistoryProvider] erst starten, wenn Auth + E-Mail-Check stabil (s. [_ensureDeferredFirestoreStreamsOnce]).
  String? _deferredFirestoreStreamsBootstrappedUid;
  ...
  void _ensureDeferredFirestoreStreamsOnce() {
    final authUser = FirebaseAuth.instance.currentUser;
    if (authUser != null &&
        !widget.skipEmailVerification &&
        !_emailVerifiedFirestoreOverride &&
        _userNeedsEmailVerificationBarrier(authUser) &&
        _emailVerificationCheckedUid != authUser.uid) {
      return;
    }
    final key = authUser?.uid ?? '';
    if (_deferredFirestoreStreamsBootstrappedUid == key) return;
    _deferredFirestoreStreamsBootstrappedUid = key;
    unawaited(() async {
      try {
        await ActivePartyService.initialize();
      } catch (e) {
        debugLog('⚠️ ActivePartyService.initialize (deferred): $e');
      }
      try {
        HistoryProvider().initialize();
      } catch (e) {
        debugLog('⚠️ HistoryProvider.initialize (deferred): $e');
      }
    }());
  }

  Future<void> _maybeRunPartyDataCleanupForAdmin() async {
    await Future<void>.delayed(const Duration(milliseconds: 600));
    if (!mounted) return;
    final um = UserService().currentUser.value;
    if (um == null) return;
    if (!AppConfig.isAdminRole(um)) return;
    try {
      await PartyCleanupService.runCleanupOnce();
    } catch (e) {
      debugLog('⚠️ PartyCleanupService.runCleanupOnce: $e');
    }
  }
  ...
  Future<bool> _runAdminBiometricGate(
    ...
    try {
      ...
      if (signOutOnFailure) {
        ...
        debugPrint(
          '⚠️ STOPP: Hier wäre ein automatischer Logout passiert! Grund: Admin-Biometrie nicht erfolgreich (signOutOnFailure, try-Zweig).',
        );
      }
      return false;
    } catch (e) {
      ...
      if (signOutOnFailure) {
        ...
        // await FirebaseAuth.instance.signOut();
        debugPrint(
          '⚠️ STOPP: Hier wäre ein automatischer Logout passiert! Grund: Admin-Biometrie-Exception (signOutOnFailure, catch-Zweig).',
        );
      }
      return false;
    } finally {
  ...
  /// Erster Firestore-User-Snapshot oder fertige Rollen-ID-Sets → erneute Rollenlogik.
  void _onRoleValidationBaselineChanged() {
    if (!mounted) return;
    _onUserModelChanged();
  }

  /// Trial/Pro/Kulanz kann sich kurz nach dem User-Dokument aus History aktualisieren — Tabs neu bauen.
  void _onSessionProStatusChanged() {
    if (!mounted) return;
    setState(() {});
  }

  /// Reagiert auf Änderungen am zentralen User-Modell: Rolle **nur** aus [role_id]
  /// vs. AppConfig-IDs (kein roles.name); Session-Lock verhindert Rollenwechsel im Betrieb.
  void _onUserModelChanged() async {
    ...
    if ((prev == null) != (userModel == null)) {
      ...
      if (prev.hasCompletedProfile != userModel.hasCompletedProfile ||
          ...
          prev.planType != userModel.planType ||
          prev.trialUntil != userModel.trialUntil ||
          prev.trialUsed != userModel.trialUsed ||
          prev.isPro != userModel.isPro ||
          prev.proUntil != userModel.proUntil) {
        shellNeedsRebuild = true;
      }
    }
    ...
    String? roleName;
    if (user != null && userModel != null) {
      // Kein Auto-Logout: erst warten, bis Firestore-Snapshot da ist und role_id lesbar ist.
      if (!UserService().isReady) {
        if (mounted) setState(() {});
        return;
      }
      final rid = userModel.roleId?.trim() ?? '';
      if (rid.isEmpty) {
        if (mounted) setState(() {});
        return;
      }
      if (_sessionLockedRoleId != null && _sessionLockedRoleId != rid) {
        _sessionLockedRoleId = rid;
      }

      final kind = AppConfig.classifyRoleId(rid);
      if (AppConfig.coreRoleIdsConfigured && kind == AppRoleKind.unknown) {
        debugLog(
          '👤 Rolle aus role_id noch nicht zuordenbar — warten (kein Auto-Logout).',
        );
        if (mounted) setState(() {});
        return;
      }
      ...
      if (kind != AppRoleKind.unknown) {
        _sessionLockedRoleId ??= rid;
      }

      if (rid == _lastRoleId) {
        if (prev?.notifyNewWishes != userModel?.notifyNewWishes ||
            prev?.enableNotificationSound != userModel?.enableNotificationSound) {
      ...
      roleName = AppConfig.labelForRoleKind(kind);
    } else {
      roleName = null;
    }
    ...
    setState(() {
      ...
      if (roleName == 'Admin') {
        ...
        if (preferredStartView == 'guest_area') {
          // WICHTIG: Admin-Konto darf NIEMALS [_currentViewRole == 'Gast'] bekommen — sonst Gast-IndexedStack/„Gast-Account“.
          // Bevorzugte Gast-Ansicht = DJ-Shell ohne Admin-View (Navigation/Start wie DJ, Rolle bleibt Admin in Daten).
          _currentViewRole = 'DJ';
          wantsAdminView = false;
          return;
        }
    ...
    // Batterie-Hinweis nur bei eindeutiger DJ-Rolle (Notification-Rechte: DJ-Einstellungen).
    if (roleName != null &&
        (roleName == 'DJ' || roleName == 'Admin' || roleName == 'Location') &&
        user != null &&
  ...
  void initState() {
    ...
    UserService().userDocSnapshotReadyNotifier.addListener(
      _onRoleValidationBaselineChanged,
    );
    AppConfig.roleDocIdSetsHydrationReadyNotifier.addListener(
      _onRoleValidationBaselineChanged,
    );
    UserService().trialExpiryPromptNotifier.addListener(
      _onTrialExpiryPromptNotifier,
    );
    UserService().sessionProStatus.addListener(_onSessionProStatusChanged);
    _onUserModelChanged();
    ...
    _authSubscription = authStateChangesDistinctByUid().listen((user) {
      if (!mounted) return;
      // Rollen-Validierung: Warte-Logik ohne Auto-Logout bei fehlender role_id / laden.
      ...
      if (accountSwitched) {
        ...
        if (user != null) {
          ...
          setState(() {
            ...
            _emailVerificationCheckedUid = null;
            _emailVerificationCheckScheduled = false;
            _emailVerificationCheckInProgress = false;
            _deferredFirestoreStreamsBootstrappedUid = null;
            EmailVerificationGateSync.clear();
            wantsAdminView = false;
            _adminBiometricVerifiedUid = null;
            _adminUseAuthenticatorFallback = false;
    ...
    // Batterie-Hinweis im DJ-Bereich – siehe _requestDjPermissionsIfNeeded()
  ...
  /// DJ-Wunsch-Benachrichtigungen aktiv (nicht im Admin-Dashboard).
  bool _isDjWishNotificationShellActive() {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return false;
    final rn = _currentRoleName;
    if (rn == null || rn == 'Gast') return false;
    final um = UserService().currentUser.value;
    if (um != null && AppConfig.isAdminRole(um)) {
      return _currentViewRole == 'DJ';
    }
    return rn == 'DJ' || rn == 'Location';
  }

  /// Globale Admin-Ankündigung: nur im DJ-Bereich, nicht im Gast-Bereich / für reine Gäste.
  ...
  void _syncDjWishNotificationGate() {
    DjWishNotificationService.instance.setShellGate(
      allow: _isDjWishNotificationShellActive(),
    );
  }
  ...
  /// Batterie-Optimierung: halbtransparenter Barrier — Dashboard bleibt sichtbar.
  ...
  void dispose() {
    ...
    AppConfig.roleDocIdSetsHydrationReadyNotifier.removeListener(
      _onRoleValidationBaselineChanged,
    );
    UserService().trialExpiryPromptNotifier.removeListener(
      _onTrialExpiryPromptNotifier,
    );
    UserService().sessionProStatus.removeListener(_onSessionProStatusChanged);
    _linkSubscription?.cancel();
    _recognitionNotificationTapSubscription?.cancel();
    DjWishNotificationService.instance.setShellGate(allow: false);
    for (final controller in _adminOtpControllers) {
      controller.dispose();
    }
  ...
  Future<void> _performEmailVerificationCheck(String uid) async {
    if (EmailVerificationGateSync.consumeLoginReloadCompleted(uid)) {
      if (mounted) {
        setState(() => _emailVerificationCheckedUid = uid);
      }
      return;
    }
    if (_emailVerificationCheckInProgress) return;
    _emailVerificationCheckInProgress = true;
    try {
      final u = FirebaseAuth.instance.currentUser;
      if (u == null || u.uid != uid) {
        if (mounted) {
      ...
      if (mounted) {
        setState(() {
          _emailVerificationCheckedUid = uid;
        });
      }
    } catch (_) {
      if (mounted) {
  ...
  Widget _buildProfileVerificationLoadingOverlay(BuildContext context) {
    final l = AppLocalizations.of(context)!;
    return Scaffold(
      backgroundColor: Colors.transparent,
      body: Stack(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    l.profile_loading_verification,
                    textAlign: TextAlign.center,
                    style: const TextStyle(
                      color: Colors.white,
                      fontSize: 16,
  ...
  Widget build(BuildContext context) {
    ...
    if (!widget.skipEmailVerification &&
        ...
        authUser != null &&
        _userNeedsEmailVerificationBarrier(authUser)) {
      final needsServerCheck = _emailVerificationCheckedUid != authUser.uid;
      if (needsServerCheck) {
        if (!_emailVerificationCheckScheduled &&
            !_emailVerificationCheckInProgress) {
          _emailVerificationCheckScheduled = true;
          WidgetsBinding.instance.addPostFrameCallback((_) {
            _emailVerificationCheckScheduled = false;
            unawaited(_performEmailVerificationCheck(authUser.uid));
          });
        }
        return _withRootBackScope(
          _buildProfileVerificationLoadingOverlay(context),
        );
      }
      return _withRootBackScope(
    ...
    if (user != null && !_announcementCheckDone) {
      ...
      if (rn != null) {
        ...
        } else if (_shouldShowGlobalAnnouncementPopup()) {
          _announcementCheckDone = true;
          WidgetsBinding.instance.addPostFrameCallback((_) async {
            await Future.delayed(const Duration(milliseconds: 2500));
            if (!mounted) return;
            if (!_shouldShowGlobalAnnouncementPopup()) return;
    ...
    if (isWaitingForProfile) {
      ...
      // Ungewöhnlicher Rollen-String: ohne Auto-Logout weiter warten / Spinner
      if (roleName != null &&
          roleName != 'Admin' &&
          roleName != 'DJ' &&
          roleName != 'Location' &&
                  ...
                  if (isDJMode) ...[
                    ...
                    ListTile(
                      ...
                      leading: Icon(
                        ...
                        size: 20,
                        color: _currentIndex == djQuickstartIndex
                            ? UIConstants.appOrange
                            : Colors.white,
                      ),
                      title: Text(
                        localizations.dj_quickstart_nav,
                        style: TextStyle(
                          fontSize: 14,
                          color: Colors.white,
                          fontWeight: _currentIndex == djQuickstartIndex
                              ? FontWeight.bold
                              : FontWeight.normal,
                        ),
                      ),
                      selected: _currentIndex == djQuickstartIndex,
                      selectedTileColor:
                          UIConstants.appOrange.withValues(alpha: 0.2),
                      onTap: () {
                        _onIndexChanged(djQuickstartIndex);
                        Navigator.pop(context);
                      },
                    ),
                  ],
                ...
                if (user == null) ...[
                  ...
                  ListTile(
                    ...
                    leading: Icon(
                      Icons.logout,
                      size: 20,
                      color: isDJMode ? Colors.red : Colors.white,
                    ),
                    title: Text(
                      ...
                      style: TextStyle(
                        ...
                        color: isDJMode ? Colors.red : Colors.white,
                        fontWeight:
                            isDJMode ? FontWeight.w500 : FontWeight.normal,
                      ),
                    ),
                    onTap: () async {
                      Navigator.pop(context);
